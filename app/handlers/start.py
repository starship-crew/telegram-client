...
async def cmd_start(message: types.Message):
    ...
    if db.get_api_token(user_id):
        await message.reply(
            "Извини но у тебя уже есть экипаж.\
 Одному пользователю можно иметь только <b>один</b> экипаж.",
            parse_mode="HTML",
            reply_markup=kb,
        )
    else:
        await NewCerw.crew_name.set()
        await message.answer(render_template("start.j2"), parse_mode="HTML")
...
async def get_crew_name(message: types.Message, state: FSMContext):
    ...
    await message.answer(
        render_template("sucsess_start.j2", content), parse_mode="HTML", reply_markup=kb
    )
